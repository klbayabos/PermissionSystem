		...
		<br>
			<div class="container" style="padding:0;">
				<table>
					...
					@foreach($ots as $ots)
						<tr><td>{{ date("m/d/Y", strtotime($ots->starting_date)) }} - {{ date("m/d/Y", strtotime($ots->end_date)) }}</td><td>{{ date('h:i A', strtotime($ots->starting_time)) }}- {{ date('h:i A', strtotime($ots->end_time)) }}</td><td>{{ date("m/d/Y", strtotime($ots->created_at)) }}</td><td>{{ $ots->state }}<br><a href="/otdetails/{{ $ots->request_id }}">View Details </a> | <a href="/delete_ot/{{ $ots->request_id }}" Onclick="return confirm('Are you sure you want to delete this request?')"> Delete</a></td></tr>
					@endforeach
				</table>
			</div>
	@endif
